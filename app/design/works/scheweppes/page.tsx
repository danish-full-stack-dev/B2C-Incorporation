  ...
  const stages = [
    {
      ...
      hours: "2 days",
      tags: ["Sketching(digital)", "Sketching(hand-drawn)"],
      highlightTag: "Sketching(digital)",
    },
    {
      icon: <Palette className="w-6 h-6" />,
      title: "3D Modeling",
      hours: "7 days",
      tags: ["Modeling", "Texturing", "Lightning", "Draft"],
      highlightTag: "Modeling",
    },
    {
      icon: <Loader className="w-6 h-6" />,
      title: "Product rendering",
      hours: "1 day",
      tags: ["Animation", "Frame-By-Frame rendering", "VFX"],
      highlightTag: "Animation",
    },
  ];
  return (
    <div className="bg-black/90">
      ...
      <div className="mx-auto w-10/12 pt-10">
          ...
          <div className="w-full  flex flex-row justify-between gap-20">
            <div className="w-4/6 border-b border-white/20">
              <h2 className="text-5xl pb-20 border-b border-white/20">
                Schweppes is the original inventor of commercial soda water and
                the definitive, premium mixer brand used globally in adult
                beverages.
              </h2>
            </div>
          ...
          <div className="flex justify-between gap-20">
            <div className="w-4/6">
              ...
              <p className="text-xl leading-7">
                Schweppes is the original inventor of commercial soda water in
                1783. Today, it stands as a premium, iconic global brand
                renowned for high-quality mixers like tonic water and ginger
                ale. Schweppes sought a modern website redesign to reflect its
                rich heritage while appealing to contemporary consumers.
              </p>
            </div>
            ...
            <div className="h-3/5 my-auto flex ">
              ...
              <div className="w-3/4 flex justify-center">
                <h4 className="text-4xl w-4/5">
                  We are immensely proud to represent a brand with such profound
                  historical significance and market leadership. Our partnership
                  helps drive the continuous legacy of excellence and innovation
                  that defines Schweppes globally.
                </h4>
              </div>
            </div>
          </div>
        ...
        <div className="h-screen">
          <iframe
            title="vimeo-player"
            src="https://player.vimeo.com/video/1146466498?h=db0f78efc8"
            width="640"
            height="360"
        ...
        <div className="flex flex-col justify-center items-center py-16 text-black/50 bg-white/90 w-full">
          <h4 className="text-5xl py-10">Schweppes</h4>
          <p className="text-xl pb-10 w-2/4">
            Interesting fact! Schweppes is the first brand of non-alcoholic
            carbonated drinks in the world with more than two centuries of
            history.
            <br /> And this is what the advertisement of this drink could look
            like
          </p>
        </div>

        <FadeSlideIn className="w-full grid grid-flow-row grid-cols-3 gap-3 grid-rows-3">
          {Array.from({ length: 9 }).map((_, index) => (
            <img
              key={index}
              src={`/sche${index + 1}.${index > 9 ? "png" : "gif"}`}
              alt="saya image"
        ...
        <div className="flex flex-col justify-center items-center py-12 text-black/50 bg-white/90 w-full">
          <h4 className="text-5xl py-5">Stills</h4>
        </div>

        <FadeSlideIn className="w-full grid grid-flow-row grid-cols-3 gap-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <img
              key={index + 10}
              src={`/sche${index + 10}.png`}
              alt="saya image"
              className="w-full object-cover"
            />
          ))}
        ...
        <FadeSlideIn className="w-full grid grid-flow-row grid-cols-2 gap-3">
          {Array.from({ length: 2 }).map((_, index) => (
            <img
              key={index + 13}
              src={`/sche${index + 13}.png`}
              alt="saya image"
